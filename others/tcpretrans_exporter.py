...
arg = "tcpretrans_ns", "TCP丢包次数(毫秒)", ["TIME", "PID", "IP", "LADDRLPORT", "T", "RADDRRPORT"]
...
def print_ipv4_event(cpu, data, size):
    ...
    try:
        cur = db.cursor()
        sql='INSERT INTO tcpretrans(pid, ip, src_ip, src_port, dest_ip, dest_port) VALUE(%s,%s,%s,%s,%s,%s)'
        value = (str(event.pid), str(event.ip), inet_ntop(AF_INET, pack('I', event.saddr)), str(event.lport), inet_ntop(AF_INET, pack('I', event.daddr)), str(event.dport))
...
def print_ipv6_event(cpu, data, size):
    ...
    tcpconnlat_gauge.labels(
        ...
        IP = event.ip,
        T = type[event.type],
        LADDRLPORT = "%s:%d" % (inet_ntop(AF_INET6, event.saddr), event.lport),
        RADDRRPORT = "%s:%d" % (inet_ntop(AF_INET6, event.daddr), event.dport),
    ).set(1)
    try:
        cur = db.cursor()
        sql='INSERT INTO tcpretrans(pid, ip, src_ip, src_port, dest_ip, dest_port) VALUE(%s,%s,%s,%s,%s,%s)'
        value = (str(event.pid), str(event.ip), inet_ntop(AF_INET6, pack('I', event.saddr)), str(event.lport), inet_ntop(AF_INET6, pack('I', event.daddr)), str(event.dport))
        cur.execute(sql, value)
        db.commit()
    except pymysql.Error as e:
        print("表格创建失败"+str(e))
