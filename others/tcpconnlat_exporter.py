...
try:
    db = pymysql.connect(host=DBHOST, user=DBUSER, password=DBPASS, database=DBNAME, port=3306)
    print('数据库成功连接')
...
arg = "tcpconnlat_ns", "TCP连接耗时(毫秒)", ["PID", "COMM", "IP", "SADDR", "DADDR", "DPORT"]
tcpconnlat_gauge = Gauge(*arg)
...
def print_ipv4_event(cpu, data, size):
    ...
    try:
        cur = db.cursor()
        sql='INSERT INTO tcpconnlat(pid, ip, src_ip, dest_ip, src_port, dest_port, lat_time) VALUE(%s,%s,%s,%s,%s,%s,%s)'
        value = (str(event.pid), str(event.ip), inet_ntop(AF_INET, pack('I', event.saddr)), inet_ntop(AF_INET, pack('I', event.daddr)), str(event.lport), str(event.dport), str(float(event.delta_us)))
...
'''def print_ipv6_event(cpu, data, size):
    ...
    try:
        cur = db.cursor()
        sql='INSERT INTO tcpconnlat(pid, ip, src_addr, dest_addr, src_port, dest_port, lat_time) VALUE(%s,%s,%s,%s,%s,%s,%s)'
        value = (str(event.pid), str(event.ip), inet_ntop(AF_INET6, pack('I', event.saddr)), inet_ntop(AF_INET6, pack('I', event.daddr)), str(event.lport), str(event.dport), str(float(event.delta_us) / 1000))
        cur.execute(sql, value)
        db.commit()
    except pymysql.Error as e:
        print("表格创建失败"+str(e))
