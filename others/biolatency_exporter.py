...
int trace_req_done(struct pt_regs *ctx, struct request *req)
{
    ...
    struct io_data data_t;
    data_t.delta = delta;
    events.perf_submit(ctx, &data_t, sizeof(data_t));
    start.delete(&req);
    return 0;
}
"""
...
arg = "biolatency_ns", "I/O延时", ["TIME"]
biolatency_gauge = Gauge(*arg)
...
def handle_event(cpu, data, size):
    ...
    biolatency_gauge.labels(
        TIME = strftime("%H:%M:%S"),
    ).set(event.delta)

    try:
        cur = db.cursor()
        sql='INSERT INTO biolatency(latency) VALUE(%s)'
        value = (str(event.delta))
